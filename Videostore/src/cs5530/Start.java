package cs5530;


import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.sql.ResultSet;
import java.sql.ResultSetMetaData;
import java.sql.SQLException;

import databaseOps.Connector;
import databaseOps.OP;
import databaseOps.QueryResult;
import databaseOps.Select;
import dbModels.Customer;
import dbModels.Video;

public class Start {

	/**
	 * @param args
	 */
	public static void displayMenu()
	{
		 System.out.println("        Video Store Management System     ");
    	 System.out.println("1. Register User:");
    	 System.out.println("2. enter your own query:");
    	 System.out.println("3. exit:");
    	 System.out.println("4. Add Video:");
    	 System.out.println("5. Order Videos:");
    	 System.out.println("6. All Customer Information:");
    	 System.out.println("7. Increase Video Inventory:");
    	 System.out.println("8. Give Feedback for Video:");
    	 System.out.println("9. Give a Feedback a rating:");
    	 System.out.println("10. Rate a user as trusted or not:");
    	 System.out.println("11. Search for a movie:");
    	 System.out.println("12. Most useful feedback for movie");
    	 System.out.println("13. Get Statistics");

    	 System.out.println("please enter your choice:");
	}
	
	@SuppressWarnings("null")
	public static void main(String[] args) {
		System.out.println("Example for cs5530");
		Connector con=null;
		String choice;
        String sql=null;
        User user = new User();
        
        int c=0;
        
        //login
         try
		 {
			//remember to replace the password
			 	 con= new Connector();
	             System.out.println ("Database connection established");
	         
	             BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
	             while(true)
	             {
	            	 while (user.id < 0)
	            		try{
            				Customer customer = new Customer();
	            			System.out.println("Please enter your login information:");
	            			System.out.println("If you do not have a login, enter 'register'.");
	            			user.login = in.readLine();
	            			if (user.login.compareToIgnoreCase("register") == 0){
		   	            		 customer.firstName = enterData("please enter a first name:", in);
		   	            		 customer.lastName = enterData("please enter a last name:", in);
		   	            		 customer.phone = enterData("please enter a phone:", in);
		   	            		 customer.address = enterData("please enter an address:", in);
		   	            		 customer.login = enterData("please enter a login:", in);
		   	            		 customer.password = enterData("please enter a password:", in);
		   	            		 QueryResult<Customer> custInfo = Options.registerUser(con.stmt, customer);
		   	            		 customer = custInfo.firstOrDefault();
		   	            		 user.id = custInfo.getAutoGeneratedKey();
		   	            		 user.login = customer.login;
		   	            		 System.out.println(custInfo.resultStr);
	            			}
	            			else{
	            				customer = Select.from(customer).where("Login", OP.Equal, user.login)
	            						.execute(con.stmt).firstOrDefault();
	            				System.out.println("Enter your password:");
	            				String pass = in.readLine();
	            				if (customer == null || customer.password.compareTo(pass) != 0)
	            					throw new SQLException();
	            				user.id = customer.ID;
	            			}
	            		}catch(SQLException e){
	            			//e.printStackTrace();
	            			System.out.println("Invalid login");
	            			continue;
	            		}
	            	 displayMenu();
	            	 while ((choice = in.readLine()) == null && choice.length() == 0);
	            	 try{
	            		 c = Integer.parseInt(choice);
	            	 }catch (Exception e)
	            	 {
	            		 
	            		 continue;
	            	 }
	            	 if (c<1 | c>20)
	            		 continue;
	            	 if (c==1)
	            	 {
	            		 Customer customer = new Customer();

	            		 customer.firstName = enterData("please enter a first name:", in);
	            		 customer.lastName = enterData("please enter a last name:", in);
	            		 customer.phone = enterData("please enter a phone:", in);
	            		 customer.address = enterData("please enter an address:", in);
	            		 customer.login = enterData("please enter a login:", in);
	            		 customer.password = enterData("please enter a password:", in);
	            		 customer.Role = enterData("please enter a role: ", in);

	            		 System.out.println(Options.registerUser(con.stmt, customer));
	            	 }
	            	 else if (c==2)
	            	 {	 
	            		 System.out.println("please enter your query below:");
	            		 while ((sql = in.readLine()) == null && sql.length() == 0)
	            			 System.out.println(sql);
	            		 ResultSet rs=con.stmt.executeQuery(sql);
	            		 ResultSetMetaData rsmd = rs.getMetaData();
	            		 int numCols = rsmd.getColumnCount();
	            		 while (rs.next())
	            		 {
	            			 for (int i=1; i<=numCols;i++)
	            				 System.out.print(rs.getString(i)+"  ");
	            			 System.out.println("");
	            		 }
	            		 System.out.println(" ");
	            		 rs.close();
	            	 }
	            	 else if (c==4)
	            	 {
	            		 Video video = new Video();

	            		 video.ISBN = enterData("please enter an isbn:", in);
	            		 video.Title = enterData("please enter a title:", in);
	            		 video.Subject= enterData("please enter a subject:", in);
	            		 video.Rating = enterData("please enter a rating:", in);
	            		 video.Price = enterData("please enter a price:", in);
	            		 video.Format = enterData("please enter a format:", in);
	            		 video.YearOfProduction = enterData("please enter a year of production:", in);
	
	            		 System.out.println(Options.addVideo(con.stmt, video));
	            	 }
	            	 else if (c==5)
	            	 {
	            		 VideoOperations vidOp = new VideoOperations();
	            		 System.out.println(vidOp.allVideos(con.stmt, new Video()));
	            		 String selectedVids = enterData("please enter the videos you wish to order in the " +
	            		 		"following format: isbn,quantity ; isbn,quantity", in);
	            		 String ccNum = enterData("please enter a major credit card number", in);
	            		 String[] videos = selectedVids.split(";");
	            		 System.out.println(Options.orderVideos(con.stmt, user.id, videos, ccNum));
	            	 }
	            	 else if (c==6)
	            	 {
	            		 String userID = enterData("Please enter the Login of the customer you would like to search for:", in);
	            		 System.out.println(Options.userRecord(con.stmt, userID));
	            	 }
	            	 else if (c==7)
	            	 {
	            		 String isbn = enterData("Please enter the ISBN of the video you would like increase the inventory:", in);
	            		 String count  = enterData("Please enter the number of videos to add to inventory:", in);
	            		 System.out.println(Options.addToInventory(con.stmt, isbn, Integer.parseInt(count)));
	            	 }
	            	 else if (c==8)
	            	 {
	            		 String isbn = enterData("Please enter the ISBN of the video you would like to score:", in);
	            		 String score  = enterData("Please enter the score between 1-10 to give the movie:", in);
	            		 String comment = enterData("Please enter a comment for your feedback", in);
	            		 System.out.println(Options.addFeedback(con.stmt, user.id, isbn, score, comment));
	            	 }
	            	 else if (c==9)
	            	 {
	            		 int fid = Integer.parseInt(enterData("Please enter the Feedback Id of the feedback you would like to rate:", in));
	            		 int rating  = Integer.parseInt(enterData("Please enter a rating between 0-2 on how useful the rating is, 0 is least useful:", in));
	            		 System.out.println(Options.addRating(con.stmt, user.id, fid, rating));
	            	 }
	            	 else if (c==10)
	            	 {
	            		 int cidTo = Integer.parseInt(enterData("Please enter the Customer Id of the customer you would like to rate:", in));
	            		 int rating  = Integer.parseInt(enterData("Please enter a rating of 0 for 'Not-trusted' and 1 for 'Trusted':", in));
	            		 System.out.println(Options.addTrustedRating(con.stmt, user.id, cidTo, rating));
	            	 }
	            	 else if (c==11)
	            	 {
	            		 System.out.println("Please search in the following comma separated order, 'Actor/Director/Title/Rating' and use 'AND/OR' to filter your results:");
	            		 String search = enterData("Example: 'johnny depp, or jj abrams, and pirates, and r'\n" +
	            		 		"commas should still be used for empty search options: ,,,pg-13\n", in);
	            		 int order = Integer.parseInt(enterData("How would you like your data sorted?\n" +
	            		 		"Year = 1\n" +
	            		 		"Average Feedback Score = 2\n" +
	            		 		"Average Trusted Feedback Score = 3", in));
	            		 System.out.println(Options.movieSearch(con.stmt, search, order));
	            	 }
	            	 else if (c==12)
	            	 {
	            		 String isbn = enterData("Please enter the ISBN of the video you would like to search for:", in);
	            		 int count = Integer.parseInt(enterData("How many results would you like?", in));
	            		 System.out.println(Options.searchMostUseful(con.stmt, isbn, count));
	            	 }
	            	 else if (c==13)
	            	 {
	            		 int videoCount = Integer.parseInt(enterData("Enter how many video you would like to list:", in));
	            		 int directorCount	 = Integer.parseInt(enterData("Enter how many directors you would like to list:", in));
	            		 int actorCount = Integer.parseInt(enterData("Enter how many actors you would like to list:", in));
	            		 System.out.println(Options.getStatistics(con.stmt, videoCount, directorCount, actorCount));
	            	 }
	            	 else
	            	 {   
	            		 System.out.println("Remeber to pay us!");
	            		 con.stmt.close(); 
	        
	            		 break;
	            	 }
	             }
		 }
         catch (Exception e)
         {
        	 e.printStackTrace();
        	 System.err.println ("Cannot connect to database server");
         }
         finally
         {
        	 if (con != null)
        	 {
        		 try
        		 {
        			 con.closeConnection();
        			 System.out.println ("Database connection terminated");
        		 }
        	 
        		 catch (Exception e) { /* ignore close errors */ }
        	 }	 
         }
	}
	
	@SuppressWarnings("null")
	private static String enterData(String line, BufferedReader in) throws IOException{
		String returnStr;
		System.out.println(line);
		while ((returnStr = in.readLine()) == null && returnStr.length() == 0);
		
		return returnStr;
	}
	
	
	
	
}



